...
#define FAILERROR(code) do { OnFfmpegError(code); exit(1); } while (0)
...
int main(int argc, char **argv)
{
  ...
  while(True) {
    ...
    if (ret < 0)
      exit(1);
    Afra.image = XGetImage(Afra.display, Afra.window, 0, 0, Afra.width, Afra.height, AllPlanes, ZPixmap);
    for (int j = 0; j < Afra.height; j++) {
	for (int i = 0; i < Afra.width; i++) {
	    // Extract RGB components from the pixel
	    unsigned long pixel = XGetPixel(Afra.image, i, j);
	    uint8_t red = (pixel & Afra.image->red_mask) >> 16;
	    uint8_t green = (pixel & Afra.image->green_mask) >> 8;
	    uint8_t blue = (pixel & Afra.image->blue_mask);
  ...
  if (Afra.codec->id == AV_CODEC_ID_MPEG1VIDEO || Afra.codec->id == AV_CODEC_ID_MPEG2VIDEO)
